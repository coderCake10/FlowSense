...
|---|---|---|---|---|
| DB-1 | `operations.settings` (plus a seed row `informational_alert_clear_time`) | ✓ **Fixed in 4b** (`configuration` app, seeded). Was: no model and no table. | The Settings API and alert auto-clear can't work | Add a model and migration, seed the row in a data migration |
