...
| Check | Result |
|---|---|
| `npm run models:check`: all present | OK/OK, exit 0 |
...
| `npm run qa:models` (dev server with `VITE_MODELS_BASE_URL=/no-such-models`) | 3/3: clear message, the missing file is named in development, and the kiosk list and route panel still work |
