...
|---|---|---|
| Type check | `pnpm run check` | Exit code 0 |
| Lint | `pnpm run lint` | 0 errors. Existing warnings are tolerated, but no new warnings in touched files. |
| Unit tests | `pnpm test` | All pass |
| Build | `pnpm run build` | Succeeds |
| Browser QA | `pnpm run qa:stepN` for this step, plus every earlier step's suite (regression) | All checks pass; this step's suite passes on 3 consecutive runs (flake check) |
...
also run `pnpm run qa:step4a` end to end against the real backend.
