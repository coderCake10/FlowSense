...
|---|---|---|
| `pnpm run check` (tsc) | Pass | Pass |
| `pnpm run lint` | 0 errors, 403 warnings | 0 errors, 386 warnings (unused imports removed from the auth page) |
| `pnpm test` | No tests existed | 27 passed |
| `pnpm run build` | Pass | Pass |
| `pnpm run qa:step1` | n/a | 36/36, three runs in a row |
...
cd frontend
pnpm install
pnpm exec playwright install chromium        # first time only, on your machine
pnpm exec vite --host 127.0.0.1 --port 3000 &
VITE_API_BASE_URL=/api/v1 pnpm exec vite --host 127.0.0.1 --port 3002 --strictPort &
pnpm run qa:step1                            # screenshots land in e2e/output/
```
