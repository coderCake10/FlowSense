...
cd frontend && VITE_API_BASE_URL=/api/v1 npx vite --host 127.0.0.1 --port 3000
BACKEND_LOG=/tmp/flowsense-django.log npm run qa:step4a

# Regression suites need an unconfigured frontend (no VITE_API_BASE_URL):
npx vite --host 127.0.0.1 --port 3005
BASE_URL=http://127.0.0.1:3005 npm run qa:step1
BASE_URL=http://127.0.0.1:3005 npm run qa:step2
```
