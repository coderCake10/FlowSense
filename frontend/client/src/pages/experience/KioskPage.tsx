...
/** Pause after typing before searching (each search is logged). */
const SEARCH_DELAY_MS = 600;

type RouteStatus = "ready" | "loading" | "unmapped" | "unreachable" | "failed";

/** A new phone-handoff session: new id, issued now (new QR and expiry). */
...
export function KioskPage() {
  ...
  const endVisitorSession = useKioskVisitorSession(kiosk);
  useEffect(() => {
    let timer: ReturnType<typeof setTimeout>;
    const reset = () => {
      clearTimeout(timer);
      timer = setTimeout(() => {
        endVisitorSession("idle_timeout");
  ...
  // Live: every room from the Map API, searched through the Search API.
  // Without an API (or before the directory loads) the built-in demo list.
  const directory = useDirectory(building);
  const [query, setQuery] = useState("");
  useEffect(() => {
    const timer = setTimeout(() => setQuery(search.trim()), SEARCH_DELAY_MS);
    return () => clearTimeout(timer);
  }, [search]);
  const found = useRoomSearch(
    building,
    directory.data,
    query,
    visitorSessionId
  ...
  const searching =
    live && !!search.trim() && (query !== search.trim() || found.isFetching);
  const shown: Destination[] = live
    ? search.trim()
      ? (found.data ?? [])
      : directory.data!.rooms
    : building.destinations.filter(d =>
        `${d.name} ${d.code}`.toLowerCase().includes(search.toLowerCase())
      );
  const [routeStatus, setRouteStatus] = useState<RouteStatus>("ready");
  const picking = useRef<string | null>(null);
  /** Shows a destination; a live room first gets its route from the
   * Navigation API (logged for Analytics). */
  const show = (destination: Destination, navigating: boolean) => {
    setSelected(destination);
    setActive(navigating);
    setKeyboard(false);
    picking.current = destination.id;
    if (!destination.roomId || destination.legs) {
      setRouteStatus("ready");
      return;
    }
    setRouteStatus("loading");
    // The built-in hand-placed route for this room, when there is one: used
    // while the room (or the kiosk) isn't connected on the map yet, so the
    // original first-floor routes keep working before annotation.
    const builtIn = building.destinations.find(
      item => item.code === destination.code && item.points.length >= 2
    );
    const settle = (routed: Destination | null, status: RouteStatus) => {
      if (picking.current !== destination.id) return;
      const shown =
        routed ??
        (builtIn
          ? {
              ...
              modelFloor: builtIn.modelFloor ?? building.kioskFloor,
            }
          : null);
      if (shown) {
        setSelected(shown);
        setQueue(q => q.map(item => (item.id === shown.id ? shown : item)));
      }
      ...
      <main className="grid min-h-[calc(100vh-72px)] md:h-[calc(100vh-72px)] md:min-h-0 grid-cols-1 md:grid-cols-[340px_minmax(0,1fr)] xl:grid-cols-[400px_minmax(0,1fr)]">
        <aside className="overflow-y-auto border-r-2 border-[#f4c542] bg-[#0b1f3a] p-4 text-white shadow-[8px_0_24px_rgba(11,31,58,.16)]">
          ...
          <p className="mt-5 text-[10px] font-medium uppercase tracking-[.1em] text-white/48">
            {searching
              ? "Searching…"
              : `${shown.length} Destinations · ${live ? "by floor" : "sorted by building"}`}
          </p>
          ...
          <div className="space-y-2">
            ...
            {!shown.length && !searching && (live || !directory.isLoading) && (
                  ...
                  <p className="mt-1 text-sm text-[#718398]">
                    ...
                    {routeStatus === "loading"
                      ...
                      : routeStatus === "unmapped"
                        ? "This room isn't on the map yet, so there's no route to show."
                        : routeStatus === "unreachable"
                          ? "No walkway on the map reaches this room yet."
                          : routeStatus === "failed"
