...
//   VITE_MODELS_BASE_URL=/no-such-models pnpm exec vite --host 127.0.0.1 --port 3004 --strictPort
//   pnpm run qa:models
...
check(
  ...
  shown &&
    (await alert.getByText("/no-such-models/eya-floor-1.glb").isVisible()) &&
    (await alert.getByText("pnpm run models:check").isVisible())
);
await page
